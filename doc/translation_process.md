...
`contrib/imagecoin-qt.pro` takes care of generating `.qm` (binary compiled) files from `.ts` (source files) files. It’s mostly automated, and you shouldn’t need to worry about it.
...
2. Update `src/qt/imagecoin_locale.qrc` manually or via
...
1. Open `imagecoin_en.ts` in Qt Linguist (included in the Qt SDK)
